...
const getUniqueJoinUserIdsByChatId = async (chat_id, user_id) => {
  try {
    ...
    const [senderTokens, receiverTokens] = await Promise.all([
      db.Token.findAll({
        ...
        attributes: [
          "device_token",
          "user_id",
          [db.Sequelize.literal("true"), "is_sender"]
        ],
      ...
      db.Token.findAll({
        ...
        attributes: [
          "device_token",
          "user_id",
          [db.Sequelize.literal("false"), "is_sender"]
        ],
        raw: true
      })
    ]);
    ...
    const enrichTokens = async (tokens, user_id) => {
      ...
      return tokens.map((token) => {
        ...
        const socket = sockets.find(
          (s) =>
            s.user_id === token.user_id && s.device_token === token.device_token
        );
    ...
    const allSocketsWithTokens = [
      ...enrichedSenderTokens,
      ...enrichedReceiverTokens
    ];
    const uniqueJoinUserIds = [
      ...new Set(sockets.map((token) => token.user_id))
    ];
...
function socketConfig(io) {
  io.on("connection", async (socket) => {
    ...
    socket.on("socket_register", async function (data) {
      ...
      try {
        ...
        if (existingSocketIds[0].includes(socket.id)) {
          console.log(
            `Socket ID ${socket.id} already registered for user ${user_id}`
          );
    ...
    socket.on("join_room", async function (data) {
      ...
      socket.join(chat_id);
      try {
        await emitToSockets(user_id, "join_room", {
          Message: "Successful",
          info: data
        });
      } catch (error) {
    ...
    socket.on("left_room", async function (data) {
      ...
      try {
        await emitToSockets(data.user_id, "left_room", {
          Message: "Successful",
          info: data
        });
      } catch (error) {
    ...
    socket.on("join_group", async function (data) {
      ...
      try {
        await commonService.emitToSockets(data.user_id, "join_group", {
          Message: "Successful",
          info: data
        });
    ...
    socket.on("left_group", async function (data) {
      ...
      try {
        await commonService.emitToSockets(data.user_id, "left_group", {
          Message: "group left Successful",
          info: data
        });
