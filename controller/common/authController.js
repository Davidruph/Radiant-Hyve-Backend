...
const login = async (req, res) => {
  const {
    ...
    device_type,
    password
    // role, //hide role from user input for login, because user can be login from different role and we are checking all role in backend
  } = req.body;
  ...
  try {
    ...
    if (user.is_blocked === true) {
      return res
        .status(400)
        .json({ status: 0, message: "Your account is blocked." });
    }
...
const forgotePasswor = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ status: 0, message: "Email not registered." });
    }
...
const verifyForgotePasswordOtp = async (req, res) => {
  ...
  try {
    ...
    const otpExpired =
      new Date() - new Date(user.otp_createdAt) > 1 * 60 * 1000;
...
const changePassword = async (req, res) => {
  try {
    ...
    if (!user.password) {
      return res
        .status(500)
        .json({ status: 0, message: "Password not set for this user." });
    }
    ...
    return res
      .status(200)
      .json({ status: 1, message: "Change password successfully" });
  } catch (error) {
    console.error(error);
    return res.status(500).json({
      status: 0,
...
const refreshTokenWeb = async (req, res) => {
  ...
  if (!refresh_token)
    return res
      .status(400)
      .json({ status: 0, message: "refresh_token is required" });
  ...
  try {
    ...
    const token = jwt.sign(
      { user_id: user.id, token_id: storedToken.id },
