...
const createStudent = async (req, res) => {
  ...
  try {
    ...
    console.log(
      "full_name, req.user.email, school.school_name, req.user.full_name",
      full_name,
      req.user.email,
      school.school_name,
      req.user.full_name
  ...
  try {
    ...
    if (teacher_id) {
      ...
      if (!teacher) {
        return res
          .status(404)
          .json({ status: 0, message: "Teacher not found" });
      }
    }
    ...
    await student.update({
      ...
      medical_insurance_no:
        madical_insuarance_no || student.medical_insurance_no,
...
const delteStudent = async (req, res) => {
  ...
  try {
    ...
    if (!student_id) {
      return res
        .status(400)
        .json({ status: 0, message: "student_id is required" });
    }
...
const listStudentFees = async (req, res) => {
  try {
    ...
    if (!page || !student_id) {
      return res
        .status(400)
        .json({ status: 0, message: "page and student_id is required" });
    }
    ...
    return res.status(200).json({
      status: 1,
      message: "Student fees list",
      data: invoice.rows,
      current_page: page,
      total_page: Math.ceil(invoice.count / limit)
    });
  } catch (error) {
    console.error("Error :", error);
    return res.status(500).json({
