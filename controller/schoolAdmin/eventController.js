...
const createEvent = async (req, res) => {
    ...
    try {
        ...
        if (is_principal == true) { whereCondition.role = "principal" }
        if (is_teacher == true) { whereCondition.role = "teacher" }
        if (is_parent == true) { whereCondition.role = "parent" }
        if (is_all == true) { whereCondition.role = { [Op.in]: ['school', 'principal'] } }
        ...
        for (const data of user) {
            const formattedDate = moment(event_date).format("DD MMMM YYYY");
            const notiType = "event_notification";
            const message = {
                title: "New Event Upcoming",
                body: `Event ${event_name} is scheduled for ${formattedDate}. ${about_event}`,
            };
            const Data = {
